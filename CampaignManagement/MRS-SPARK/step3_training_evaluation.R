...
##########################################################################################################################################
CM_AD_factors <-  RxXdfData(file = paste(HDFSDir, "/CMADfactorsXdf", sep=""),fileSystem = hdfs)
...
CM_AD1 <- RxXdfData(file = paste(HDFSDir, "/CMAD1Xdf", sep=""),fileSystem = hdfs, stringsAsFactors = TRUE)
...
CM_AD_Train <- RxXdfData(file = paste(HDFSDir, "/CMADTrainXdf", sep=""),fileSystem = hdfs, stringsAsFactors = TRUE)
CM_AD_Test <- RxXdfData(file = paste(HDFSDir, "/CMADTestXdf", sep=""),fileSystem = hdfs, stringsAsFactors = TRUE)
...
evaluate_model <- function(observed, predicted_probability, threshold) { 
  ...
  confusion <- base::table(observed, predicted) 
  print(confusion) 
  tp <- confusion[1, 1] 
  fn <- confusion[1, 2] 
  fp <- confusion[2, 1] 
...
Prediction_Table_RF <- RxXdfData(file = paste(HDFSDir, "/PredictionTableRFXdf", sep=""),fileSystem = hdfs, stringsAsFactors = TRUE)
...
##########################################################################################################################################
# Make Predictions, then import them into R. The observed Conversion_Flag is kept through the argument extraVarsToWrite.
Prediction_Table_GBT <- RxXdfData(file = paste(HDFSDir, "/PredictionTableGBTXdf", sep=""),fileSystem = hdfs, stringsAsFactors = TRUE)
