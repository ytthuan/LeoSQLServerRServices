<#
.SYNOPSIS
Script to train and test the product cross sell template with SQL + MLS

.DESCRIPTION
This script will show the E2E work flow of Product Cross Sell machine learning
...
if ($isAdmin -eq 'True')
{
    ...
    ##########################################################################
    # Create table ProductXSL
    ##########################################################################
    Write-Host -ForeGroundColor 'green' ("Step 1: Create and populate table in Database {0}" -f $DatabaseName)
    if($Prompt -ne 'N') {
        #Prompt user
    ...
    if ($ans -eq 'y' -or $ans -eq 'Y')
    {
        ...
        {
            # create tables
            Write-Host -ForeGroundColor 'green' ("Create SQL table: ProductXSL")
            $script = $scriptPath + "create_tables.sql"
            ExecuteSQLScript $script
            
            $tableNameFile = "ProductXSL"
            Write-Host -ForeGroundColor 'green' ("Populate SQL table: $tableNameFile")
            $destination = $SolutionData + $tableNameFile + ".csv"
            Write-Host -ForeGroundColor 'magenta'("    Populate SQL table: $tableNameFile")
            $tableName = $DatabaseName + ".dbo." + $tableNameFile
            $tableSchema = $SolutionData + $tableNameFile + ".xml"
                
            ExecuteBCP("bcp $tableName format nul -c -x -f $tableSchema -t ','")
            Write-Host -ForeGroundColor 'magenta'("    Loading {0} to SQL table..." -f $tableNameFile)
            ExecuteBCP("bcp $tableName in $destination -t ',' -f $tableSchema -F 2 -C 'RAW' -b 20000")
            Write-Host -ForeGroundColor 'magenta'("    Done...Loading {0} to SQL table..." -f $tableNameFile)
        }
        catch
        {
            Write-Host -ForegroundColor DarkYellow "Exception in populating table:"
    ...
    ##########################################################################
    # Create stored procedure and executing
    ##########################################################################
    Write-Host -ForeGroundColor 'green' ("Step 2: Creating recommendations")
    if($Prompt -ne 'N') {
        #Prompt user
        $ans = Read-Host 'Continue [y|Y], Exit [e|E], Skip [s|S]?'
        if ($ans -eq 'E' -or $ans -eq 'e')
    ...
    if ($ans -eq 'y' -or $ans -eq 'Y')
    {
        try
        {
            # execute create_storedprocedure.sql
            Write-Host -ForeGroundColor 'magenta'("    Creating stored procedure...")
            $script = $scriptPath + "create_storedprocedure.sql"
            ExecuteSQLScript $script

            #execute the stored procedure just created
            Write-Host -ForeGroundColor 'magenta'("    Executing stored procedure...")
            $query = "EXEC create_recommendations '$connectionString'"
            ExecuteSQL $query

        }   
        catch
        {
            Write-Host -ForegroundColor DarkYellow "Exception in creating recommendations:"
            Write-Host -ForegroundColor Red $Error[0].Exception 
            throw
        }
    }
